...
def bulk_save(records):
    """Áp trạng thái tự động (computed_status) cho tất cả record."""
    conn = get_conn()
    for r in records:
        conn.execute(
...
if not filtered:
    ...
    with b1:
        ...
        fc = sum(1 for r in filtered if r["computed_status"] == "cho_duyet")
        st.markdown(
            f"<div style='font-size:12.5px;color:#555;padding:8px 0;'>"
            f"Áp trạng thái <i>tự động</i> cho <b>{len(filtered)}</b> hồ sơ đang hiển thị — "
            f"✅ <b style='color:#2e7d32;'>{fa}</b> duyệt &nbsp;"
            f"❌ <b style='color:#c62828;'>{fd}</b> từ chối &nbsp;"
            f"⏳ <b style='color:#e65100;'>{fc}</b> chờ duyệt"
            f"</div>",
            unsafe_allow_html=True)
    with b2:
        if st.button("✅ Duyệt danh sách ứng viên", use_container_width=True, key="bulk"):
            bulk_save(filtered)
            st.cache_data.clear()
